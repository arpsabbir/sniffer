...
while true; do
    ...
    inter=$( ip addr | grep inet | grep brd | head -n 1 | cut -f13 -d" " )
...
client_interface=$( ip addr | grep inet | grep brd | head -n 1 | awk '{print $9}' )
inp="ping.txt"
...
my_interface=$( ip addr | grep inet | grep brd | head -n 1 | awk '{print $9}' )
echo -e "${yellow}+------------------------------------------------------------------------------+${off}"
...
sleep 3
interface=$( ip addr | grep inet | grep brd  | awk '{print $9}' | head -n 1 )
...
-ci | --Consultation_info :${blue} Showing Configuration Details.${off}
-l  | --liste_addr : ${blue}Show ALL IP in Same Network .${off}
-p  | --ping_request :${blue} Detection des packets ICMP Envoyées Vers Ta Machine. ${off}
-a  | --arp_scan :${blue} Detection ARP poisoning attack.  ${off}
-d  | --dns_spoof_scan :${blue} Detection DNS SPOOF attack.  ${off}
-b  | --build_index :${blue} Build a HTML index and send it to The Administation Server.  ${off}
-C  | --catch_dhcp :${blue} Check and catch Every New Client Request a DHCP DISCOVER.  ${off}
-as | --arp_sniffer :${blue}Start ARP SNIFER ==> built with libpcap ©.${off}
-ts | --tcp_sniffer : ${blue} Start a TCP sniffer 'sniff all packet'==> built with libpcap © .${off}
-sc | --start_compile : ${blue} Compile All C files .${off}
${blue}+------------------------------------------------------------------------------------+${off}
"
