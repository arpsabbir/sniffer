...
sed -i "5s/${catchit5}/ON SERVER/" config.txt
...
l=0
    ...
    thisisit=$( cat arp_addr.txt | grep -n '' | grep "^$1" | sed -E "s/[1-9]://g"  | tail -n 1 )
        if [ $k == 2 ]; then
...
if [ $k -le 1 ]; then
    ...
    echo -n "do you want to bloc traffic for this spoofer ? Y/N : "
    read choix
    if [ $choix == "yes" ]; then
...
</html>" >> /var/www/html/icmp_diag.html
            ;;       
